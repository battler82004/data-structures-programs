...
// Determines the number of leaves in a tree starting at a node.
template <class elemType>
int binaryTreeType<elemType>::leavesCount(nodeType<elemType> *p)
    const {

    if ((p->lLink == nullptr) && (p->rLink == nullptr)) // Base case
        return 1;
    if (p->lLink == nullptr) // No left branch (general case)
        return leavesCount(p->rLink);
    if (p->rLink == nullptr) // No right branch (general case)
